...
for file in Lemon_results Metabolites.allreg.txt Metabolites.randomreg.txt Metabolites.topreg.txt Metabolites.xml.gz TFs.allreg.txt TFs.randomreg.txt TFs.topreg.txt TFs.xml.gz clusterfile tight_clusters.txt; do
    if [ -L "$file" ]; then
...
python3 $SCRIPT_PATH         --input_dir .         --output_dir .         --run_id 2000HVG_coherence0.5_top2.0pct_clusters5         --coherence_threshold "0.5"         --regulator_selection_method "percentage"         --top_n_percent_regulators "2.0"         --regulator_fold_cutoff "2.0"         --regulator_types "TFs:Lovering_TF_list.txt,Metabolites:Metabolomics.txt"
