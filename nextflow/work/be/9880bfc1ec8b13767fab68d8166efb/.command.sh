...
if [ "proteomics" = "proteomics" ] && [ -f "/home/borisvdm/repo/LemonIte/nextflow/scripts/Preprocessing_TFA_Proteomics.R" ]; then
    ...
    Rscript $SCRIPT_PATH \
        ...
        --output_dir . \
        --regulator_types "hPTMs:Histone_proteomics.csv,Metabolites:metabolomics_combined.csv" \
        --top_n_genes "10000"             --perform_TFA "false"             --use_omics_specific_scaling "true"             --DESeq_contrast1 "Cell_line"             --design_formula "~ Cell_line"             --metadata_columns "Cell_line,FAB,MITO_subtype,Risk_classification"             --expression_col "count"             --sample_id_col "ID"             --organism "human"             $([ ! -z "$PRIOR_NETWORK" ] && echo "--prior_network $PRIOR_NETWORK")
fi
