...
normalize_organism_name <- function(organism) {
  normalized <- tolower(trimws(as.character(organism)))

  if (normalized %in% c("human", "hsa", "homo_sapiens")) {
    return("human")
  }

  if (normalized %in% c("mouse", "mmu", "mus_musculus")) {
    return("mouse")
  }

  stop(
    sprintf("Unsupported organism '%s'. Expected one of: human, mouse.", organism),
    call. = FALSE
  )
}

resolve_orgdb_package <- function(organism) {
  organism_name <- normalize_organism_name(organism)
...
read_cluster_assignments <- function(cluster_path, cluster_column) {
  cluster_df <- read.csv(cluster_path, check.names = FALSE, stringsAsFactors = FALSE)

  if (cluster_column %in% colnames(cluster_df) && "Module" %in% colnames(cluster_df)) {
    return(data.frame(
      Module = as.character(cluster_df$Module),
      MegaGO_cluster = as.character(cluster_df[[cluster_column]]),
      stringsAsFactors = FALSE
    ))
  }

  cluster_with_rownames <- read.csv(cluster_path, row.names = 1, check.names = FALSE, stringsAsFactors = FALSE)
  if (!cluster_column %in% colnames(cluster_with_rownames)) {
    stop(
  ...
  data.frame(
    Module = rownames(cluster_with_rownames),
    MegaGO_cluster = as.character(cluster_with_rownames[[cluster_column]]),
    stringsAsFactors = FALSE
  )
}
...
run_rrvgo_cluster_labels <- function(enrichment_path,
  ...
  orgdb_package <- resolve_orgdb_package(organism)
  ensure_packages(c("dplyr", "rrvgo", orgdb_package, "GOSemSim"))
  ...
  module_clusters <- read_cluster_assignments(cluster_path, cluster_column)
  ...
  if (length(clusters_to_label) > 0) {
    message(sprintf("Preparing GO semantic data once for ontology %s (%s)...", ontology, orgdb_package))
    semdata <- GOSemSim::godata(orgdb_package, ont = ontology, keytype = "ENTREZID")
...
main <- function() {
  ...
  run_rrvgo_cluster_labels(
    ...
    organism = if (!is.null(args[["organism"]])) args[["organism"]] else "human",
