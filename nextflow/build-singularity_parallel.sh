...
# Build the image
echo "Building Singularity image: lemontree-pipeline_v1.0.0_parallel.sif"
echo "Note: Building without sudo using --fakeroot (requires user namespace support)"
singularity build --fakeroot --bind "${PWD}/.singularity/tmp:/tmp" lemontree-pipeline_v1.0.0_parallel.sif Singularity_parallel.def
...
# Verify the build
if [ -f "lemontree-pipeline_v1.0.0_parallel.sif" ]; then
    echo ""
    echo "✅ Parallel Singularity image built successfully!"
    echo "Image size: $(du -h lemontree-pipeline_v1.0.0_parallel.sif | cut -f1)"
    echo ""
    cleanup_cache
    echo ""
    echo "Test the image with:"
    echo "singularity exec lemontree-pipeline_v1.0.0_parallel.sif R --version"
    echo "singularity exec lemontree-pipeline_v1.0.0_parallel.sif python3 --version"
    echo ""
