...
# Use custom EnrichR plotting when available
...
percentile <- 2
fold <- percentile  # Changed fold parameter to percentile
...
metabolite_regulators_file <- paste0(base_dir, 'ModuleViewer_files/Metabolite.percentile', percentile, '_list.txt')
...
# Source shared GSEA/EnrichR plotting utilities so both analysis modes can use plot_enrichr()
...
if (ANALYSIS_METHOD %in% c("GSEA", "both")) {
  ...
  # GSEA plotting utilities are sourced earlier so both GSEA and EnrichR can use them
        ...
        if (nrow(enrichment[[db]]) > 0) {
          ...
          tryCatch({
            if (exists("plot_enrichr") && USE_CUSTOM_ENRICHR_PLOTTING) {
              p <- plot_enrichr(enrichment[[db]], top_n = 10, numChar = 40, title = db) +
                theme(axis.text.y = element_text(size = 12), axis.text.x = element_text(size = 12),
                      panel.border = element_rect(color = "black", fill = NA, linewidth = 2))
            } else {
              p <- plotEnrich(enrichment[[db]], showTerms = 10, numChar = 40, y = 'Count', orderBy = 'P.value') +
                theme(axis.text.y = element_text(size = 12), axis.text.x = element_text(size = 12),
                      panel.border = element_rect(color = "black", fill = NA, linewidth = 2))
            }
...
run_enrichment_for_regulator <- function(reg, reg2genes, dbs, outdir) {
  tryCatch({
    ...
    for (i in dbs) {
      tryCatch({
        if (exists("plot_enrichr") && USE_CUSTOM_ENRICHR_PLOTTING) {
          plot <- plot_enrichr(enrichment[[i]], top_n = 15, numChar = 40, title = i) +
...
# Example usage
tryCatch({
  #regulator_targets_enrichment(file.path(base_dir, 'Networks', paste0('hPTMs2targets_percentile', percentile, '_selected_', n_modules, '_modules.txt')))
  regulator_targets_enrichment(file.path(base_dir, 'Networks', paste0('Metabolites2targets_percentile', percentile, '_', n_modules_name, '_modules.txt')))
  regulator_targets_enrichment(file.path(base_dir, 'Networks', paste0('TFs2targets_percentile', percentile, '_', n_modules_name, '_modules.txt')))
